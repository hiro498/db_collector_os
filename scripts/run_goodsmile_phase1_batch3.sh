#!/usr/bin/env bash
# DB Collector OS - Phase 1 batch #3 for the first production DB
# (job_prod_figure_official_site / 美少女フィギュア公式メーカーDB, Good Smile
# Company source: https://www.goodsmile.com/en/scalefigure_list).
#
# batch #1 (scripts/run_goodsmile_phase1_batch1.sh, kept as-is for history)
# proved the pipeline end to end on a single known product page. batch #2
# (scripts/run_goodsmile_phase1_batch2.sh, also kept as-is for history)
# proved list -> discover -> multi-fetch -> multi-insert works: 71 new
# entities inserted, error_count=0, DB integrity ok. But its real result
# also exposed a run-lifecycle bug: a successful run with more queue left
# to process was labeled job.status=retry (indistinguishable from an
# actual failure being retried), and a single-domain rate limit made
# _drain_fetch_queue() fetch exactly one page per run_once() call --
# turning "one batch, up to max_pages" into 73 separate runs 15 seconds
# apart. batch #3 reuses batch #1/#2's entire safety design unchanged and
# exists to prove those two bugs are actually fixed: a successful run
# never becomes status=retry (see JobStatus.CONTINUING), and one logical
# run can drain multiple pages toward max_pages
# (config.max_drain_wait_seconds, opt-in, set in the job YAML).
#
# Paste this whole file into your VPS SSH session, or run it as
# `./scripts/run_goodsmile_phase1_batch3.sh` from /root/tools/db_collector_os.
...
#
# SSH-disconnect-safe design (identical to batch #1):
...
#     unit (db-collector-phase1-batch3-goodsmile), so it keeps going even if
#     you disconnect. Re-running this script reuses/replaces that same named
#     unit rather than accumulating new ones.
#
...
# Scheduler does for every other job. concurrency stays 1 and rate_limit
# stays whatever the job YAML says (>=5s between requests to
# www.goodsmile.com) -- this script does not loosen either.
#
# Safe to re-run any time.

db_collector_phase1_batch3_goodsmile() {
    ...
    local WATCH_UNIT="db-collector-phase1-batch3-goodsmile"
    ...
    echo "===================================================================="
    echo " DB Collector OS -- Phase 1 batch #3 (Good Smile / $JOB_ID)"
    echo " Goal: list page -> discover multiple product URLs -> fetch"
    echo " multiple products -> insert multiple entities (not one fixed seed)."
    echo "===================================================================="
    ...
    # -- migrations / schema check (fresh code above may add migrations) ------
    echo; echo "--- preflight: DB schema / migrate ---"
    "$CLI" migrate || { echo "[BLOCK] migrate failed"; SKIP_START=1; }
    ...
    # -- adapter/pipeline registry after `git pull`/`git merge --ff-only` -----
    # -- above landed new/changed code (this is what caused "Unknown adapter: -
    # -- figure_official_site" and, separately, a config-seed silently never --
    # -- reaching fetch_queue, on earlier batch attempts -- a long-running ----
    ...
    echo; echo "--- preflight: worker reload (adapter/pipeline code freshness) ---"
...
"; then
        ...
        echo "        Not starting Phase 1 batch #3 now; already-running jobs are left alone."
...
" 2>/dev/null)"
    ...
    # -- other production jobs untouched --------------------------------------
    echo; echo "--- preflight: no other production job enabled by this script ---"
    local OTHER_ENABLED_BEFORE
    OTHER_ENABLED_BEFORE="$("$PY" -c "
from db_collector_os.config import load_config
from db_collector_os.database import Database
from db_collector_os.job_registry import JobRegistry
cfg = load_config('config/default.yaml')
...
print(','.join(bad))
" 2>/dev/null)"
    if [ -n "$OTHER_ENABLED_BEFORE" ]; then
        echo "[WARN] other production job(s) already enabled: $OTHER_ENABLED_BEFORE (pre-existing; this script won't touch them)"
    else
        echo "OK: no other production job currently enabled"
    fi
    ...
    # -- capture the run_id landscape AND entity count BEFORE this batch, so --
    # -- the watcher can prove afterward both that a genuinely NEW run_history -
    # -- row was created (run_history is immutable execution history) and ------
    # -- that the entity population actually grew (ENTITY_DELTA), not just -----
    # -- that one already-known entity got re-fetched.
...
" 2>/dev/null)"
    ...
    local ENTITY_COUNT_BEFORE
    ENTITY_COUNT_BEFORE="$("$PY" -c "
from db_collector_os.config import load_config
from db_collector_os.database import Database
cfg = load_config('config/default.yaml')
db = Database(cfg.db_path)
row = db.query_one('SELECT COUNT(*) AS n FROM entities WHERE job_id=? AND deleted_at IS NULL', ('$JOB_ID',))
print(row['n'] if row else 0)
" 2>/dev/null)"
    ENTITY_COUNT_BEFORE="${ENTITY_COUNT_BEFORE:-0}"
    echo "ENTITY_COUNT_BEFORE=$ENTITY_COUNT_BEFORE"

    # RUN_COUNT_BEFORE: how many run_history rows this job already has --
    # lets the watcher isolate exactly the rows THIS batch creates
    # (run_history is immutable + append-only, so "everything after this
    # count" is unambiguous) to compute RUN_COUNT/LIFECYCLE_OK and catch a
    # regression of the "one URL = one run" bug.
    local RUN_COUNT_BEFORE
    RUN_COUNT_BEFORE="$("$PY" -c "
from db_collector_os.config import load_config
from db_collector_os.database import Database
cfg = load_config('config/default.yaml')
db = Database(cfg.db_path)
row = db.query_one('SELECT COUNT(*) AS n FROM run_history WHERE job_id=?', ('$JOB_ID',))
print(row['n'] if row else 0)
" 2>/dev/null)"
    RUN_COUNT_BEFORE="${RUN_COUNT_BEFORE:-0}"
    echo "RUN_COUNT_BEFORE=$RUN_COUNT_BEFORE"

    # QUEUE_PENDING_BEFORE: how many fetch_queue rows are already 'queued'
    # for this job -- if there's real pending work before the batch even
    # starts, the watcher can check that a single run actually drained
    # multiple pages of it, not just one page per run.
    local QUEUE_PENDING_BEFORE
    QUEUE_PENDING_BEFORE="$("$PY" -c "
from db_collector_os.config import load_config
from db_collector_os.database import Database
cfg = load_config('config/default.yaml')
db = Database(cfg.db_path)
row = db.query_one(\"SELECT COUNT(*) AS n FROM fetch_queue WHERE job_id=? AND status='queued'\", ('$JOB_ID',))
print(row['n'] if row else 0)
" 2>/dev/null)"
    QUEUE_PENDING_BEFORE="${QUEUE_PENDING_BEFORE:-0}"
    echo "QUEUE_PENDING_BEFORE=$QUEUE_PENDING_BEFORE"

    "$CLI" jobs sync || { echo "[FATAL] jobs sync failed"; return 1 2>/dev/null || true; }

    # Guarantee this run's config seed_urls reach fetch_queue synchronously,
    # from THIS process -- not by depending on db-collector-worker@1.service
    # having reloaded code that would otherwise enqueue them on its next
    # run_once() tick. This process always runs whatever is on disk right
    # now (ff-only merge above already brought it up to date). Idempotent:
    # never duplicates or force-refetches an already-tracked URL.
    ...
    if systemctl is-active --quiet "$WATCH_UNIT.service" 2>/dev/null; then
        ...
        chmod +x scripts/_phase1_batch3_watch_and_report.sh scripts/_phase1_batch3_report.py 2>/dev/null || true
        systemd-run --unit="$WATCH_UNIT" \
            --description="DB Collector OS Phase 1 batch #3 watcher (Good Smile)" \
            --collect \
            ...
            --setenv=PHASE1_ENTITY_COUNT_BEFORE="$ENTITY_COUNT_BEFORE" \
            --setenv=PHASE1_RUN_COUNT_BEFORE="$RUN_COUNT_BEFORE" \
            --setenv=PHASE1_QUEUE_PENDING_BEFORE="$QUEUE_PENDING_BEFORE" \
            bash "$APP_DIR/scripts/_phase1_batch3_watch_and_report.sh" \
    ...
    echo
    echo "PRODUCTION_CRAWL_STARTED=PENDING"
    echo "(the watcher disables AND pauses the job automatically once the batch settles -- PASS,"
    echo " PARTIAL, or FAIL alike -- see var/reports/ for the full audit: seed/discovered URL"
    echo " counts, fetch queue breakdown, entity/evidence counts and delta, RUN_COUNT/"
    echo " LIFECYCLE_OK, run_history, checkpoint, HTTP status breakdown, DB integrity, and the"
    echo " final PHASE1_RESULT. The job's final status is never left as 'continuing'/'retry'.)"

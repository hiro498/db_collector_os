#!/usr/bin/env bash
# Internal helper for scripts/run_goodsmile_phase1_batch2.sh. Invoked via a
...
# full report + evaluates the Phase 1 batch #2 PASS/PARTIAL/FAIL result,
# and always disables the job afterward (success or anomaly), per the
# Phase 1 "one batch at a time" policy -- same design as batch #1's
# watcher, reused rather than reinvented.
#
# Not meant to be run directly by a human -- use
# scripts/run_goodsmile_phase1_batch2.sh, which launches this via systemd-run.
...
REPORT_FILE="$REPORT_DIR/phase1_batch2_${STAMP}.txt"

log() { echo "$*" | tee -a "$REPORT_FILE"; }

log "===================================================================="
log " Phase 1 batch #2 watcher started: $(date -u +%FT%TZ)"
log " job_id=$JOB_ID timeout=${TIMEOUT_SECONDS}s poll=${POLL_SECONDS}s"
log " entity_count_before=$ENTITY_COUNT_BEFORE"
...
log ""
"$PY" scripts/_phase1_batch2_report.py "$JOB_ID" "$ENTITY_COUNT_BEFORE" 2>&1 | tee -a "$REPORT_FILE"
...
log ""
log "===================================================================="
log " Phase 1 batch #2 -- result evaluation"
log "===================================================================="
...
# -- the "did this batch actually prove a real population-growth crawl" ---
# -- bar, evaluated only once nothing above is structurally broken. Not ---
# -- meeting it is PARTIAL (the system worked; this batch just didn't grow
# -- the population), never a silent PASS -- re-fetching one already-known
# -- entity must never be reported as success.
PARTIAL_REASONS=""
[ "${ENTITY_DELTA:-0}" -gt 0 ] 2>/dev/null || PARTIAL_REASONS="${PARTIAL_REASONS}entity_delta_not_gt_0(${ENTITY_DELTA:-unknown});"
[ "${BATCH_INSERTED:-0}" -gt 0 ] 2>/dev/null || PARTIAL_REASONS="${PARTIAL_REASONS}inserted_count_not_gt_0;"
[ "${BATCH_FETCHED:-0}" -gt 1 ] 2>/dev/null || PARTIAL_REASONS="${PARTIAL_REASONS}fetched_count_not_gt_1;"

if [ -n "$FAIL_REASONS" ]; then
    log "PHASE1_RESULT=FAIL"
    log "FAIL_REASONS=$FAIL_REASONS"
    log "READY_FOR_BATCH3=NO"
    ...
    log "READY_FOR_BATCH3=YES"
fi

log ""
log "Disabling job as designed (one batch at a time -- success, partial, or"
log "anomaly alike -- see docs/first_production_db.md)."
...
log ""
log "===================================================================="
log " Phase 1 batch #2 watcher finished: $(date -u +%FT%TZ)"

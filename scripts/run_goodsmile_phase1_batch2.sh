#!/usr/bin/env bash
# DB Collector OS - Phase 1 batch #2 for the first production DB
...
# proved the pipeline end to end on a single known product page plus fixed
# the run-lifecycle, seed-idempotency, and config-seed-guarantee bugs that
# blocked real discovery. batch #2 reuses that entire safety design
# unchanged and exists to prove the actual Phase 1 goal: list page ->
# discover multiple product URLs -> fetch multiple products -> insert
# multiple entities -- not a single fixed seed.
#
# Paste this whole file into your VPS SSH session, or run it as
# `./scripts/run_goodsmile_phase1_batch2.sh` from /root/tools/db_collector_os.
...
#     unit (db-collector-phase1-batch2-goodsmile), so it keeps going even if
...
db_collector_phase1_batch2_goodsmile() {
    ...
    local WATCH_UNIT="db-collector-phase1-batch2-goodsmile"
    ...
    echo "===================================================================="
    echo " DB Collector OS -- Phase 1 batch #2 (Good Smile / $JOB_ID)"
...
"; then
        ...
        echo "        Not starting Phase 1 batch #2 now; already-running jobs are left alone."
...
" 2>/dev/null)"
    ...
    if systemctl is-active --quiet "$WATCH_UNIT.service" 2>/dev/null; then
        ...
        chmod +x scripts/_phase1_batch2_watch_and_report.sh scripts/_phase1_batch2_report.py 2>/dev/null || true
        systemd-run --unit="$WATCH_UNIT" \
            --description="DB Collector OS Phase 1 batch #2 watcher (Good Smile)" \
            --collect \
            ...
            bash "$APP_DIR/scripts/_phase1_batch2_watch_and_report.sh" \
    ...
    echo
    echo "PRODUCTION_CRAWL_STARTED=PENDING"
    echo "(the watcher disables the job automatically once the batch settles -- PASS, PARTIAL,"
    echo " or FAIL alike -- see var/reports/ for the full audit: seed/discovered URL counts,"
    echo " fetch queue breakdown, entity/evidence counts and delta, run_history, checkpoint,"
    echo " HTTP status breakdown, DB integrity, and the final PHASE1_RESULT.)"
